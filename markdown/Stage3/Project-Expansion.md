...
**What's still missing:** Nothing — all 7 modules (A-G) are complete. Autoscaling, progressive delivery, log aggregation, Redis caching, queue-based processing, alerting, and GitOps are all deployed.
...
> **Status: COMPLETE** — Prometheus Adapter installed in `monitoring` namespace. HPA deployed via Helm chart (1-3 replicas). Custom metric `requests_per_second` mapped from `http_request_duration_seconds_count`. Load test verified: 1 → 2 → 3 pods (scale up under 100 req/s), 3 → 1 (scale down when idle). ArgoCD `ignoreDifferences` prevents HPA/ArgoCD replica conflicts.
...
Prometheus Adapter bridges Prometheus metrics into the Kubernetes Custom Metrics API. Without it, HPA can only use CPU/memory (which requires `metrics-server`, disabled in our Minikube). With it, HPA can scale on any Prometheus metric — like HTTP request rate.
...
Created `task-manager/scripts/prometheus-adapter-values.yaml` with custom rules:

```yaml
# Point at our kube-prometheus-stack Prometheus instance
prometheus:
  url: http://monitoring-kube-prometheus-prometheus.monitoring.svc
  port: 9090

# Default rules = true gives us CPU/memory resource metrics
# (replaces metrics-server functionality)
rules:
  default: true
  custom:
    # Map http_request_duration_seconds_count → requests_per_second
    # This is our prom-client HTTP histogram from src/lib/metrics.ts
...
> **Note:** The adapter image (`registry.k8s.io/prometheus-adapter/prometheus-adapter:v0.12.0`) had intermittent TLS timeouts pulling from `registry.k8s.io`. Workaround: pull with Docker Desktop, then `minikube image load`.

Verify the custom metrics API is registered:

```bash
# Should return requests_per_second and task_operations
...
#### Step 2: Add HPA Template

Created `task-manager/helm-chart/templates/hpa.yaml` — conditional on `.Values.autoscaling.enabled`:
...
Key decisions:
- **`type: Pods` + `AverageValue`**: HPA divides the metric sum across pods. Target of 10 means "scale up when each pod averages >10 req/s".
- **Scale-up policy**: 100% per 15s (aggressive — can double pod count quickly).
- **Scale-down stabilization**: 60s before scaling down (prevents flapping).

#### Step 3: Modify Deployment Template

HPA and Deployment must not both manage `replicas` — they'd conflict. When autoscaling is enabled, the Deployment template omits the `replicas` field entirely, letting HPA own it:
...
#### Step 4: Add values.yaml Section
...
#### Step 5: ArgoCD ignoreDifferences

Since ArgoCD manages the Deployment via GitOps but HPA dynamically changes `replicas`, ArgoCD would see the live replica count as "drift" and try to revert it. The `ignoreDifferences` field tells ArgoCD to ignore the replicas field:
...
#### Step 6: Load Test & Verify

Created `task-manager/scripts/load-test.js` (k6 script). Since k6 isn't installed locally and DNS resolution from the host to in-cluster services is complex, the load test was run from **inside a pod** in the cluster:

```bash
# Launch a pod with network access to the app
kubectl run loadtest --rm -it --image=node:22-slim -- bash

# Inside the pod — install k6-compatible load generator
# Actually used a Node.js script hitting the FQDN:
npx -y autocannon -c 100 -d 120 \
  http://task-manager.task-manager.svc.cluster.local:3000/api/tasks
```

> **Key debugging insight:** The initial load test used the short DNS name `task-manager` (no namespace suffix). This resolved to the `default` namespace, not `task-manager` — all requests silently failed. The `.catch()` handler still incremented the counter, so metrics appeared but no actual traffic hit the app. Fix: use the **FQDN** `task-manager.task-manager.svc.cluster.local`.

Watch HPA in action:

```bash
kubectl get hpa -n task-manager -w
# TARGETS column shows current/target — when current > target, scale up fires

kubectl describe hpa -n task-manager
# Events section shows scaling decisions:
# "New size: 2; reason: pods metric requests_per_second above target"
# "New size: 1; reason: All metrics below target"
```

**Verified results:**
- **Scale UP**: 1 → 2 → 3 pods under ~100 req/s load (metric exceeded target of 10)
- **Scale DOWN**: 3 → 1 after load stopped (60s stabilization window respected)
- **ArgoCD**: Stayed Synced/Healthy throughout (ignoreDifferences working correctly)



### New K8s Resources

- `HorizontalPodAutoscaler/task-manager` — Autoscaling rule (1-3 replicas, custom metric)
- `Deployment/prometheus-adapter` (in `monitoring` namespace) — Metrics API bridge
- `APIService/v1beta1.custom.metrics.k8s.io` — Registers custom metrics API endpoint
- ConfigMap `prometheus-adapter` — Custom metric mapping rules
...
> **Status: COMPLETE** — Argo Rollouts controller installed. Rollout replaces Deployment with NGINX traffic splitting (20% → 50% → 100%). AnalysisTemplate queries Prometheus success rate. Verified: successful canary promotion (20% → analysis pass → 50% → analysis pass → 100%) AND automatic abort (empty metrics → query error → rollback). HPA retargeted to Rollout.
...
Replace the Deployment with an **Argo Rollout** that uses a canary strategy: route 20% of traffic to the new version, analyze Prometheus metrics, then gradually promote to 100%.

### What You'll Learn

- **Progressive delivery**: Canary deployment strategy with weighted traffic shifting
- **Rollout CRD**: Argo Rollouts' replacement for Deployment with advanced strategies
- **AnalysisTemplate**: Automated Prometheus-based promotion/rollback decisions
- **NGINX traffic splitting**: Weighted routing via canary Ingress cloning
- **ArgoCD + Rollouts coexistence**: `ignoreDifferences` for dynamic Rollout-managed state
...
```
                         ...
                         │                             │
                    Revision 1 (stable)          Revision 2 (canary)
                    ReplicaSet: rs-aaa           ReplicaSet: rs-bbb
                    Image: app:latest            Image: app:v2-canary
                         │                             │
    Service: task-manager (stable)    Service: task-manager-canary
                         │                             │
                    Ingress: task-manager     Cloned Ingress (auto)
                    (90% traffic)             (canary-weight: 20→50→100)
                         │                             │
                         └────── NGINX Ingress ───────┘
                                    │
                              task-manager.local
...
Also install the kubectl plugin (for `kubectl argo rollouts` commands):
```bash
# Windows: download kubectl-argo-rollouts-windows-amd64 from GitHub releases
# Linux/Mac: curl -LO https://github.com/argoproj/argo-rollouts/releases/latest/download/kubectl-argo-rollouts-linux-amd64
```

#### Step 2: Create AnalysisTemplate

The AnalysisTemplate defines a Prometheus query that checks the HTTP success rate (non-5xx responses / total responses). It runs 3 times at 30s intervals. If the success rate drops below 95%, the analysis fails and the canary aborts.
...
metadata:
  name: task-manager-success-rate
spec:
  metrics:
    - name: success-rate
      interval: 30s           # Query every 30s
      count: 3                # Run 3 checks total
      successCondition: result[0] >= 0.95
      failureLimit: 1         # Abort after 1 failed check
      consecutiveErrorLimit: 5  # Tolerate transient Prometheus errors
      provider:
        prometheus:
          address: http://monitoring-kube-prometheus-prometheus.monitoring.svc:9090
          query: |
            (
              sum(rate(http_request_duration_seconds_count{namespace="task-manager",status_code!~"5.."}[2m]))
              /
              sum(rate(http_request_duration_seconds_count{namespace="task-manager"}[2m]))
            ) or vector(1)    # Default to 1.0 when no traffic (avoids empty result error)
```

> **Key insight:** The `or vector(1)` fallback is critical. Without it, when there's no traffic, Prometheus returns an empty vector, and `result[0]` fails with "slice index out of range". The `consecutiveErrorLimit` also helps tolerate transient network issues.

#### Step 3: Create Rollout Template

The Rollout replaces the Deployment when `canary.enabled: true`. It uses the same pod spec but adds a canary strategy with NGINX traffic routing.
...
spec:
  ...
  strategy:
    canary:
      canaryService: task-manager-canary      # Service pointing to canary pods
      stableService: task-manager             # Service pointing to stable pods
      trafficRouting:
        nginx:
          stableIngress: task-manager          # Existing Ingress (cloned for canary)
      steps:
        - setWeight: 20                        # 20% traffic to canary
        - pause: { duration: 30s }             # Wait for traffic to flow
        - analysis:                            # Check success rate
            templates:
              - templateName: task-manager-success-rate
...
NGINX traffic splitting works by:
1. The existing Ingress (`task-manager`) routes to the **stable service** (90% initially)
2. Argo Rollouts **clones** this Ingress with a `nginx.ingress.kubernetes.io/canary-weight: 20` annotation, routing to the **canary service**
3. NGINX sees both Ingresses for the same hostname and splits traffic by weight
4. As the canary progresses (20 → 50 → 100), Rollouts updates the canary-weight annotation

#### Step 4: Create Canary Service

A separate Service for canary pods. Argo Rollouts dynamically updates its selector to point to the canary ReplicaSet.
...
#### Step 5: Make Deployment Conditional

The Deployment template only renders when canary is **disabled**:
```yaml
...
#### Step 6: HPA Targets Rollout

When canary is enabled, HPA targets the Rollout instead of the Deployment:
```yaml
scaleTargetRef:
  {{- if .Values.canary.enabled }}
  apiVersion: argoproj.io/v1alpha1
...
#### Step 7: ArgoCD ignoreDifferences

ArgoCD self-healing would revert the dynamic changes that Rollouts makes to Service selectors and Rollout replicas. The `ignoreDifferences` rules tell ArgoCD to ignore these fields:
...
ignoreDifferences:
  ...
  - group: ""
    kind: Service
    name: task-manager        # Rollouts modifies selector dynamically
    jsonPointers:
      - /spec/selector
  - group: ""
    kind: Service
...
```yaml
canary:
  enabled: false  # Default off; set to true to use Rollout instead of Deployment
  analysis:
    interval: 30s
    count: 3
    successRate: 0.95
...
#### Successful Canary Promotion (happy path)
1. Triggered new revision (image tag `latest` → `v2-canary`)
2. Canary ReplicaSet created, 20% traffic routed via NGINX canary Ingress
3. Background traffic generator (~5 req/s) seeded Prometheus metrics
4. **Analysis #1** (at 20%): 3/3 checks passed, success rate = 100%
5. Promoted to 50% traffic
6. **Analysis #2** (at 50%): 3/3 checks passed, success rate = 100%
7. Promoted to 100%, revision 2 became stable, revision 1 scaled down

#### Automatic Abort (demonstrated rollback)
- First attempt failed because there was no traffic → empty Prometheus result → `result[0]` "slice index out of range" → 5 consecutive errors → analysis failed → **rollout automatically aborted**, traffic reverted to stable revision

### Promote or Abort Manually

```bash
# Promote canary to next step (skip pause/analysis)
...
- `Rollout/task-manager` — Progressive delivery controller (replaces Deployment)
- `AnalysisTemplate/task-manager-success-rate` — Prometheus success-rate check
- `AnalysisRun/*` — Ephemeral analysis instances (one per analysis step)
- `Service/task-manager-canary` — Canary traffic target
- `Deployment/argo-rollouts` (in `argo-rollouts` namespace) — Rollouts controller
- Canary Ingress (auto-created clone of stable Ingress with canary-weight annotation)
...
What you get:      - Pods scale up under load (1 → 3 replicas)
                   - Pods scale down when idle (save resources)
                   - Custom metric scaling (not just CPU-based)
                   - ArgoCD ignoreDifferences prevents HPA/ArgoCD conflicts
...
What you get:      - 20% canary → analyze → 50% → analyze → 100%
                   - Automatic rollback if success rate < 95%
                   - Zero-downtime releases (no more "all pods restart at once")
                   - NGINX weighted traffic splitting via canary Ingress cloning
