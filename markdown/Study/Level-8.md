...
redis:
  enabled: false    # Originally disabled by default; now enabled in the refactored values.yaml
  image:
    repository: redis
    pullPolicy: IfNotPresent    # Now set to "Never" (Minikube local images)
    tag: 7-alpine
  persistence:
    size: 1Gi
  resources:
...
> **Note:** After the `setup-cluster.sh` refactor, all services (including Redis) have `enabled: true` and `pullPolicy: Never` by default in `values.yaml`. The original defaults shown above illustrate the conditional rendering concept — Redis was initially optional and had to be explicitly enabled via `--set redis.enabled=true`.
