...
19. [The Label Selector Bug (Service AND Deployment)](#the-label-selector-bug-service-and-deployment)
...
33. [The Docker Build and Load Workflow](#the-docker-build-and-load-workflow)
...
async function run() {
  ...
  for (const template of due) {
    // Create a task from the template, stamping recurringTaskId for traceability
    await prisma.task.create({
      data: {
        ...
        recurringTaskId: template.id,  // Links task back to its template
      },
    });

    // Advance nextRun based on cron expression
    const interval = cronParser.parseExpression(template.cron, { currentDate: new Date() });
...
Every Deployment's pod template and Service selector includes a unique component label. This is **critical** — without it, the main app Service selector would match notification pods too (see [The Label Selector Bug](#the-label-selector-bug-service-and-deployment)).
...
## The Label Selector Bug (Service AND Deployment)
...
Later, a second variant surfaced: `kubectl exec deployment/task-manager -- <command>` was executing inside a **Meilisearch** pod instead of the app pod, because the Deployment's `.spec.selector` was equally too broad.
...
### Variant 1: Service Selector (traffic routing)
...
### Variant 2: Deployment Selector (`kubectl exec` targeting wrong pod)

Weeks later, the **same bug** was found on the Deployment's `.spec.selector`. The main app Deployment's selector was also missing the `component` label:

```yaml
# task-manager deployment selector (BEFORE fix)
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: task-manager       # ← matches ALL pods!
      app.kubernetes.io/instance: task-manager    # ← matches ALL pods!
      # Missing: app.kubernetes.io/component: app
```

This caused `kubectl exec deployment/task-manager -- <command>` to land in a **Meilisearch** pod instead of the app pod. `kubectl exec` uses the Deployment's selector to find pods — with a too-broad selector, it picks any matching pod (often alphabetically or by creation order).

**Diagnosis**:
```bash
# Verify which pod kubectl exec targets
kubectl exec deployment/task-manager -- hostname
# BEFORE fix: meilisearch-0  (wrong!)
# AFTER fix:  task-manager-xxx (correct)
```

### The Fix: Component Labels Everywhere

Add a unique `app.kubernetes.io/component` label to **both** the Service selector AND the Deployment selector (and pod template labels):

```yaml
# Main app Service (AFTER fix)
selector:
  app.kubernetes.io/name: task-manager
  app.kubernetes.io/instance: task-manager
  app.kubernetes.io/component: app          # ← only matches main app pods
...
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: task-manager
      app.kubernetes.io/instance: task-manager
      app.kubernetes.io/component: app      # ← critical for kubectl exec targeting
  template:
    metadata:
      labels:
        app.kubernetes.io/name: task-manager
        app.kubernetes.io/instance: task-manager
        app.kubernetes.io/component: app    # ← pod must carry this label too
...
> **Warning**: Deployment selectors are **immutable**. You cannot add a label to `.spec.selector.matchLabels` on an existing Deployment — `kubectl apply` / `helm upgrade` will error. You must **delete** the Deployment and recreate it:
> ```bash
...
# Verify kubectl exec targets the right pod
kubectl exec deployment/task-manager -- hostname
# AFTER fix: task-manager-xxx (correct)
```
...
**Every service's Deployment `.spec.selector`, pod template labels, AND Service selector must include a unique `app.kubernetes.io/component` label.** This prevents:
- **Service routing** to wrong pods (traffic load-balanced across services)
- **`kubectl exec`** targeting wrong pods (commands execute in unintended containers)

The full list of component labels:
- Main app: `app`
...
**Cause**: The main app Service selector matches notification pods too (shared base labels). Traffic is load-balanced across both pods — Fastify returns 404 for Next.js routes. A related variant: the Deployment `.spec.selector` is also too broad, causing `kubectl exec` to target wrong pods.
...
# Verify kubectl exec targets the right pod
kubectl exec deployment/task-manager -- hostname
# If it returns a non-app pod name, the Deployment selector is too broad
```

**Solution**: Add a unique `app.kubernetes.io/component` label to each service's Deployment `.spec.selector`, pod template labels, AND Service selector. Note: Deployment selectors are immutable — you must delete and recreate the Deployment. See [The Label Selector Bug](#the-label-selector-bug-service-and-deployment).
...
- Service AND Deployment label selectors and collision risks
...
- Debugging label selector collisions (Service traffic routing AND `kubectl exec` targeting wrong pod)
...
## The Docker Build and Load Workflow

### Docker Desktop Is the Default (Not a Fallback)

All images are built with Docker Desktop (the host Docker daemon), then loaded into Minikube. This is the default workflow, not a fallback — Docker Desktop has more memory (~16GB+ vs Minikube's ~7GB shared daemon), builds are faster, and OOM kills on large dependency trees are eliminated.

```bash
# 1. Build with Docker Desktop (from task-manager/, NOT services/<name>/)
docker build -t ralf090102/<service>:latest -f services/<service>/Dockerfile .

# 2. CRITICAL: Force-remove old image from Minikube before loading
...
`minikube image load` doesn't overwrite images with the same tag. If `ralf090102/<service>:latest` already exists in Minikube, loading a new image with the same tag silently keeps the **old** image. The pod continues running old code.
...
> **Note**: CronJobs don't support `kubectl rollout restart`. New scheduler images are picked up on the next scheduled run, or trigger manually:
> ```bash
...
- Docker Desktop build → force-remove → `minikube image load` workflow
