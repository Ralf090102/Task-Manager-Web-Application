...
## Canary Deployments — Stage 3 Module G

- **Purpose**: Progressive delivery — route a percentage of traffic to new versions, analyze Prometheus metrics, and automatically promote or rollback
- **Argo Rollouts controller**: Installed in `argo-rollouts` namespace via `install.yaml` (not Helm)
  - kubectl plugin: `kubectl-argo-rollouts` binary in user home dir (Windows: `C:\Users\ralfh\kubectl-argo-rollouts.exe`)
- **Rollout CRD**: Replaces `kind: Deployment` with `kind: Rollout` when `canary.enabled: true`
  - Canary strategy: 20% → pause 30s → analysis → 50% → pause 30s → analysis → 100%
  - NGINX traffic routing: `canaryService`, `stableService`, `trafficRouting.nginx.stableIngress`
  - Rollouts clones the stable Ingress with `canary-weight` annotation for NGINX traffic splitting
- **AnalysisTemplate**: `templates/analysis-template.yaml` — Prometheus success-rate query
  - Query: non-5xx / total request rate, with `or vector(1)` fallback for no-traffic
  - Config: 3 checks at 30s intervals, success >= 95%, failureLimit 1, consecutiveErrorLimit 5
  - **CRITICAL**: Without `count` field, metric runs indefinitely (invalid for step analysis)
  - **CRITICAL**: Without `or vector(1)` fallback, empty Prometheus result causes "slice index out of range"
- **Services**: Existing `task-manager` = stable service, new `task-manager-canary` = canary service
  - Rollouts controller dynamically modifies both Service selectors (pod-template-hash labels)
- **HPA interaction**: `scaleTargetRef` conditionally targets Rollout vs Deployment
  ```yaml
  scaleTargetRef:
    ...
    {{- end }}
  ```
- **Deployment conditional**: `{{- if not .Values.canary.enabled }}` — Deployment only renders when canary disabled
- **ArgoCD `ignoreDifferences`**: Rollout replicas + Service selectors (Rollouts modifies these dynamically)
  - Rollout `/spec/replicas` (HPA manages when autoscaling enabled)
  - Service `task-manager` `/spec/selector` (Rollouts adds pod-template-hash)
  - Service `task-manager-canary` `/spec/selector`
- **values.yaml**: `canary:` section (`enabled: false` default, `analysis:` with interval/count/successRate/failureLimit)
- **Traffic generation for analysis**: Must have background traffic during canary for Prometheus data
  - Run from in-cluster pod: `kubectl run traffic-gen --image=node:22-slim --restart=Never --command -- node -e "setInterval(()=>fetch('http://task-manager.task-manager.svc.cluster.local:3000/api/tasks').catch(()=>{}),200)"`
- **Verified**: Successful canary (20% → analysis pass → 50% → analysis pass → 100%) AND automatic abort (no traffic → query error → rollback → retry succeeded)
- Common commands:
  ```bash
  # Watch live rollout
  ...
  # Promote to next step (skip pause/analysis)
  ...
  # Trigger new canary revision (change image tag)
  kubectl argo rollouts set image task-manager task-manager=ralf090102/task-manager-app:v2-canary -n task-manager
  ```
